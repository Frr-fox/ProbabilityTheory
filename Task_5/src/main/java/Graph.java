...
public class Graph {
    ...
    private final CategoryChart histogram;
    ...
    public void drawPolygonOfFrequencies() {
        ...
        dataX.add(result);
        dataY.add((double) countNumbers);
        drawAxis(chartPolygon, new double[]{dataX.get(0), dataX.get(0)}, new double[]{5.0, 0.0},
                new double[]{dataX.get(0), dataX.get(dataX.size() - 1)}, new double[]{0.0, 0.0});
    ...
    public void drawBarGraph() {
        ...
        dataX.add(result);
        dataY.add(countNumbers / h);
        CategorySeries series = histogram.addSeries("Гистограмма частот", dataX, dataY);
    }
